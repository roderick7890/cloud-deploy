...
| `lyquid-info-client` | `src/utils/request/lyquid-info-client.ts` | active | Resolves a Lyquid ID to its deployed contract address through `GetLyquidInfo` | `fetchLyquidContractAddress` | Off-chain sender, Deploy step update detection | Returns `null` when the Lyquid has no visible contract; throws detailed network/RPC errors |
